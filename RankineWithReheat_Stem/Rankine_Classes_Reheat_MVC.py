...
class rankineView():
    ...
    def updateOptimizedValues(self, model):
        """
        Updates the GUI elements related to P_Mid and T_Mid to reflect optimized values.
        """
        self.le_PMid.setText(f"{model.p_mid:.3f}")  # Assuming le_PMid is a QLineEdit for P_Mid
        self.le_TMid.setText(f"{model.t_mid:.3f}")  # Assuming le_TMid is a QLineEdit for T_Mid
    def MakeCanvas(self):
            """
...
class rankineController():
    ...
    def optimize(self):
        def objFn(vars):
            ...
            vars[0] = P_Mid
            vars[1] = T_Mid
            Computes the cycle thermal efficiency and includes penalties for constraints violations.
            """
            p_mid = vars[0]
            t_mid = vars[1]
            penalties = 0

            # Update the model's mid pressure and temperature
            ...
            # Penalty for P_Mid going out of allowable range
            if p_mid >= self.Model.p_high or p_mid <= self.Model.p_low:
                penalties += 1000 * (abs(p_mid - self.Model.p_high) + abs(p_mid - self.Model.p_low))

            # Penalty for T_Mid going out of allowable range
            if t_mid > self.t_mid_max or t_mid < self.t_mid_min:
                penalties += 1000 * (abs(t_mid - self.t_mid_max) + abs(t_mid - self.t_mid_min))

            # Penalty for negative heat added in the reheat process
            if self.Model.heat_added_2 < 0:
                penalties += 10000  # Large penalty for negative heat added

            return 100.0 - eff + penalties

        # Initial guess for P_Mid and T_Mid
        initial_guess = np.array([
            self.Model.p_low + 0.01 * (self.Model.p_high - self.Model.p_low),  # P_Mid initial guess
            (self.t_mid_max + self.t_mid_min) / 2  # T_Mid initial guess
        ])

        # Read conditions from GUI before optimizing
        self.readConditionsFromGUI()

        # Optimize efficiency by adjusting P_Mid and T_Mid
        result = minimize(objFn, initial_guess, method="Nelder-Mead")

        # Apply optimized P_Mid and T_Mid
        self.Model.p_mid = result.x[0]
        self.Model.t_mid = result.x[1]
        self.le_PMid.setText(f"{self.Model.p_mid:.3f}")
        self.le_TMid.setText(f"{self.Model.t_mid:.3f}")

        self.updateModel()

    # Additional methods might be necessary to define t_mid_max and t_mid_min
    ...
    def updateModel(self):
        """
        Update model calculations and refresh the view
        """
        self.View.updateOptimizedValues(self.Model)
#endregion